...
;;; ----------------------------------------------------------------------
;;; Main loop. This loop ensures that handle_sample is called 100 times.
;;; ----------------------------------------------------------------------
	set r31,100
loop
	call handle_sample
...
loopin	
	...
	ld0 r1,(current_location) ; load r1 from DM0(current location)-----pointers to ringbuffer
	nop
	move ar1,r1               
  ...
  ;;; -----------------------------
    ...
    st1 (ar1),r0 ;new sample is stored in DM1(ar1) ringbuffer
    nop

    repeat endloop,31
    convss acr0,(ar0++),(ar1++%)
endloop
    move r5,ar1 ;save pointers values ar1 to r5
    convss acr0,(ar0++),(ar1++%)
    st0 (current_location),r5 ;save r5 to DM0(current_location) for next incoming sample 
    nop
